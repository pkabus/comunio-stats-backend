...
public class FlatPlayerSnapshotServiceIntegrationTest {
	...
	void getOrSave_thenReturnPlayerSnapshot() {
		...
		softAssert.assertThat(savedPlayer.get()).usingRecursiveComparison().ignoringFields("id").isEqualTo(playerEntity);
		...
		Page<PlayerSnapshotEntity> playerSnapshots = playerSnapshotService.findByPlayerId(savedPlayer.get().getId(),
				PageRequest.of(0, 10));
		softAssert.assertThat(playerSnapshots).usingElementComparatorIgnoringFields("id").containsExactly(playerSnapshotEntity);
	...
	void getOrSaveTwice_thenReturnPlayerSnapshot() {
		...
		Page<PlayerSnapshotEntity> playerSnapshots = playerSnapshotService.findByPlayerId(savedPlayer.get().getId(),
				PageRequest.of(0, 10));
		softAssert.assertThat(playerSnapshots).usingElementComparatorIgnoringFields("id").containsExactly(playerSnapshotEntity0);
